make_student_links_district_pdf_le <- function(dataset, pagenum){
  
  district_byStudent <- dataset %>% 
    group_by(schoolID, classID, School, TeacherLast, Grade, StudentID) %>%
    select(schoolID, classID, StudentID, Timepoint, Language, Grade, LastName, School, TeacherLast, SEL.SS, UO.SS,SPS.SS, SC.SS)
  
  names(district_byStudent) <- c("schoolID", "classID", "StudentID", "Timepoint", "Language", "Grade", "Last Name", "School", "Teacher",
                                 "Overall SEL", "Understanding Others",
