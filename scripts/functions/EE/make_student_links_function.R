make_student_links <- function(dataset, pagenum){
  ...
  if(pagenum == 1){
    student_links <- classroom_byStudent %>% ungroup %>% 
      mutate(StudentID = paste0("<a href=", tolower(gsub(" ", "_", Teacher)), "/", tolower(gsub(" ", "_", StudentID)),"_index.html>", StudentID, "</a>"))
    
  }
  else if(pagenum == 2){
    student_links <- classroom_byStudent %>% ungroup %>% 
      mutate(StudentID = paste0("<a href=", tolower(gsub(" ", "_", Teacher)), "/", tolower(gsub(" ", "_", StudentID)),"_index.html#timepoint-2>", StudentID, "</a>"))
  }
